...
export const mockPerformanceApplications: PerformanceApplication[] = [
  {
    id: "1",
    ...
    description:
      "Core payment processing service handling all transaction requests",
  },
  {
    id: "2",
    ...
    lastChecked: new Date().toISOString(),
    description:
      "Authentication and authorization service for all applications",
  },
  {
    id: "3",
...
export const getPerformanceApplicationDetails = (
  id: string,
): PerformanceApplicationDetails => {
  const app = mockPerformanceApplications.find((a) => a.id === id);
  if (!app) {
    throw new Error("Application not found");
  }

  const details: PerformanceApplicationDetails = {
    ...app,
...
const generateReasoning = (app: PerformanceApplication): string => {
  const issues: string[] = [];

  if (app.responseTime > 2000) {
    issues.push(
      "• **Database Query Performance**: Slow queries detected, likely due to missing indexes or inefficient joins",
    );
    issues.push(
      "• **Memory Pressure**: High memory utilization causing garbage collection delays",
    );
  } else if (app.responseTime > 800) {
    issues.push(
      "• **Network Latency**: Increased latency in downstream service calls",
    );
  }

  if (app.errorRate > 5) {
    issues.push(
      "• **Connection Pool Exhaustion**: Database connections maxed out during peak load",
    );
    issues.push(
      "• **Timeout Issues**: Requests timing out before completion",
    );
  } else if (app.errorRate > 2) {
    issues.push(
      "• **Intermittent Failures**: Occasional timeouts under load",
    );
  }

  if (app.throughput < 1000 && app.status !== "healthy") {
    issues.push(
      "• **Resource Constraints**: CPU or memory bottlenecks limiting throughput",
    );
  }
